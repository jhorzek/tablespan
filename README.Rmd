...
approach to creating tables in R.
...
`tabelle` is not yet available from CRAN. To install from GitHub, run the following
lines:
...
Often, we may want to replace the technical names (e.g., `mean_hp` and `sd_hp`) 
used in `R` when exporting to .xlsx.  In the example above, we may want
to replace `mean_hp` and `sd_hp` with "Mean" and "SD". In 
`tabelle` renaming variables is achieved with `new_name:old_name`.
...
While `tabelle` provides limited styling options, some elements can be adjusted.
For example, we may want to print some elements in bold or format numbers differently.
In `tabelle`, styling happens when translating the table to an `openxlsx` workbook
with `write_tab`.
To this end, `tabelle` provides a `style
...
`tabelle` also allows formatting specific data types. Let's assume that we want
